          ...
          <div className="w-full flex flex-col gap-2 mt-2">
            {accounts.map((acc) => (
              <button
                key={acc.id}
                onClick={() => {
                  setCurrentName(acc.name)
                  setCurrentId(acc.id)
                  setOpen(false)
                }}
                className="flex items-center justify-between w-full py-4 px-4 rounded-xl font-semibold transition text-gray-900 border-2 border-transparent bg-gray-100 hover:bg-gray-200"
              >
